    ...
    std::thread thread([&]() {
        ...
        while (true) {
            ...
            if (!img.empty() && (i % frame_skip == 0)) {
                ...
                SLAM.feed_monocular_frame(img, newTimestamp, mask);
            }
            ...
            // if (!no_sleep && i < frames.size() - 1) {
                ...
                const auto wait_time = (1.0/20.0) - track_time;
                if (0.0 < wait_time) {
            ...
            newTimestamp = newTimestamp + (1/20);
            if(forward) {
                i++;
                if(i >= frames.size()) {
                    i = frames.size()-1;
                    forward = !forward;
                    std::cout << "Play backwards" << std::endl;
                }
            }
            else {
                i--;
                if(i == 0) {
                    forward = !forward; 
                    std::cout << "Play forward" << std::endl;  
                }
            }
        }
